...
my_data = genfromtxt("../map/floorplan.csv", delimiter=',')
...
def lese_karte_von_csv(dateipfad):
    try:
        ...
        print(f"Die Datei '{dateipfad}' konnte nicht gefunden werden. Bitte überprüfen Sie den Pfad und versuchen Sie es erneut.")
        return None
...
def main():
    ...
    if modus == "E":
        ...
        karte = lese_karte_von_csv(Path(dateipfad))
    ...
    ausgang = (randdom.randint(0, breite - 1), random.randint(0, hoehe - 1))
    schritte = fine_wege_und_zeichne_karten(breite, hoehe, namen, ausgang, ausgabe_ordner, karte)
